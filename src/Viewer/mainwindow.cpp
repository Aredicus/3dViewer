...
void MainWindow::on_pushButton_clicked()
{
    ...
    ui->info->append("Lines: " + QString::number(glWidget->n_faces / 5));
...
{
    glWidget->point_size = ui->point_size->value();
    glWidget->update();
}
...
{
    glWidget->line_size = ui->line_size->value();
    glWidget->update();
}
...
{
    glWidget->point_type = ui->point_type->currentIndex();
    glWidget->update();
}
...
{
    glWidget->dotted = ui->line_type->currentIndex();
    glWidget->update();
}
